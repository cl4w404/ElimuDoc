...
class FormThree extends StatefulWidget {
  FormThree({Key? key, required this.category, required this.title}) : super(key: key);
  String category;
  String title;
    ...
    FileDownloader.downloadFile(
        ...
        onProgress: (fileName, double progress) {
          context.showErrorMessage(
              message: "${namepdf} is $progress% downloaded  ");

          print('FILE $fileName HAS PROGRESS $progress');
        },
        onDownloadCompleted: (String path) {
          context.showErrorMessage(
              message: "${namepdf} finished downloding 😁  check your Downloads folder!! ");
          print('FILE DOWNLOADED TO PATH: $path');
        },
        onDownloadError: (String error) {
          print('DOWNLOAD ERROR: $error');
        });
    Navigator.push(context, MaterialPageRoute(builder: (context)=>ViewPdf( document: _doc,)));  //Notice the Push Route once this is done.
  }
            ...
            Expanded(
              child: StreamBuilder<QuerySnapshot>(
                  ...
                  builder: (context, snapshots){
                    return (snapshots.connectionState == ConnectionState.waiting)
                    ...
                    ): ListView.builder(itemCount: snapshots.data!.docs.length,itemBuilder: (context, index) {
                      ...
                      if(name.isEmpty){
                        ...
                        return ListTile(
                          ...
                          subtitle: Text("${data['paper']} done in ${data['year']} term ${data['term']}"),
                          trailing: Container(
                              height: 50,
                              width: 50,
                              child: Icon(
                      ...
                      if(data['paper'].toString().contains(name.toLowerCase()) || data['School'].toString().startsWith(name)){
                        return ListTile(
                          onTap: (){
                            _rewardedAd!.show(
                                onUserEarnedReward:
